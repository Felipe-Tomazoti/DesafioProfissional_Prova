...
public class PersonagemValidacao {
    public void verificarPontos(Integer forca, Integer defesa) {
        if (forca < 0 || defesa < 0) {
            throw new PersonagemException("Força ou defesa do Personagem, não podem ser menor que 0!");
        }
        if (forca + defesa > 10) {
            throw new PersonagemException("A soma de força e defesa do Personagem, deve ser de no máximo 10 pontos!");
        }
        if (forca + defesa != 10) {
            throw new PersonagemException("A soma de força e defesa do Personagem, deve ser de 10 pontos!");
        }
    }
